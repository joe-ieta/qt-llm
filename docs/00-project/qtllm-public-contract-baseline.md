...
当前发布前矩阵为 Qt5/Qt6 × STATIC/SHARED 四组，每组 CTest 6/6；安装包消费者 20/20；`add_subdirectory` 消费者 4/4；公开头文件安装与编译检查通过。`v0.2.11`、`v0.2.12` 已发布，`v0.2.13` 为当前发布版本。

`v0.2.12` 起 Provider 上报的 token 用量通过 `LlmResponse::usage`（`LlmUsage`）提供；缺失用量以 `LlmUsage::available = false` 表示未知，token 计数不能按 0 解释，原有字段读取保持源码兼容。
